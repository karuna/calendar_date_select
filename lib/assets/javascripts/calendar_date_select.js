//= require calendar_date_select/string_ext
//= require calendar_date_select/date_ext
//= require calendar_date_select/date_formatter
//= require_tree ./calendar_date_select/formats
...
Date.zero_pad = function(hour) { var padded2 = parseInt(hour, 10); if (hour < 10) padded2 = "0" + padded2; return padded2; }
Date.prototype.getPaddedMinutes = function() { return Date.zero_pad(this.getMinutes()); }
...
CalendarDateSelect.prototype = {
  initialize: function(target_element, options) {
    ...
    this.options = $H({
      ...
      formatter_class: DateFormatter.Natural
    }).merge(options || {});
  ...
  formatter: function() {
    var klass = this.options.get("formatter_class");
    return (new klass());
  },
  ...
  initButtonsDiv: function()
  {
    ...
    if (this.options.get("time"))
    {
      ...
      this.minute_select = new SelectBox(buttons_div,
        blank_time.concat($R(0,59).select(function(x){return (x % that.options.get('minute_interval')==0)}).map(function(x){ return $A([ Date.zero_pad(x), x]); } ) ),
        { 
          calendar_date_select: this, 
  ...
  dayHover: function(element) {
    ...
    this.updateFooter(this.formatter().update(hover_date).toFormattedString(this.use_time));
  },
  ...
  dateString: function() {
    return (this.selection_made) ? this.formatter().update(this.selected_date).toFormattedString(this.use_time) : "&#160;";
  },
  ...
  parseDate: function()
  {
    ...
    this.date = value=="" ? NaN : this.formatter().parseFormattedString(this.options.get("date") || value);
